...
function App() {
	...
	useEffect(() => {
		//слушаем коллекцию todos принимаем все изменения
		db.collection("todos")
			.orderBy("timestamp", "desc")
			.onSnapshot((snapshot) => {
				//сортируем по датам задачи
				setTodos(
					snapshot.docs.map((doc) => ({
						id: doc.id,
						task: doc.data().task
					}))
				); //разделяем документы => берем поле todo каждого отдельного документа и записываем в локальный массив
			});
	}, []);
		...
		<div className="App">
			...
			<form>
				<FormControl>
					...
					/>{" "}
					{/* при любом изменении поля идет перезапись input */}
				</FormControl>
				...
				>
					{" "}
					{/* по клику добавляем input в массив todos и отправляем форму */}
					Add Todo {/* отключаем кнопку если input пустой */}
				</Button>
			</form>

			{/* <Button onClick = { event => (db.collection('todos').docs.map( doc => db.collection('todos').doc(doc.id).delete() ))} variant = "contained" color = "secondary">
        DELETE ALL
      </Button> */}
